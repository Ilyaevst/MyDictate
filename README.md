...
В разделе «Звуки и уведомления» для каждого из трёх способов завершения
диктовки можно выбрать отдельный системный звук или отключить его. Там же
настраиваются общая громкость и время показа синего подтверждения «Готово»
для режима буфера обмена. По умолчанию используется `Pop`, громкость 75%,
а подтверждение показывается 5 секунд.
